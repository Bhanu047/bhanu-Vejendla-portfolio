...
let liveIndex=0;
const projects={
 kingdom:{label:"FLAGSHIP PRODUCTION PLATFORM",title:"Kingdom846",text:"A gaming community needed one dependable place for alliance information, live rosters, applications, rankings, voting and operational updates. I built Kingdom846 as a responsive platform used by real community members.",decision:"I separated the React interface from an Express API, persisted operational data in SQLite and enforced role boundaries for administrators, alliance leaders and War Room users. Rate-limited authentication, version-aware updates and a server-side Royal Advisor make the platform practical to operate.",flow:[{step:"EXPERIENCE",name:"React + Vite",detail:"Responsive portal"},{step:"SERVICES",name:"Express API",detail:"Role-based access"},{step:"DATA",name:"SQLite",detail:"Live community state"},{step:"OPERATE",name:"Production site",detail:"Real users + updates"}],signals:["Role-based access","Version-aware writes","Responsive experience"],evidence:["596 visitors in first 19 days","3,298 pageviews across 49 countries","4.4 pages per visit"],evidenceLabel:"PRODUCTION EVIDENCE",tags:["React","Express","SQLite","REST APIs"],url:"https://kingdom846.com",action:"Visit live website ↗"},
 stream:{label:"STREAMING DATA PLATFORM",title:"Weblog Stream",text:"Web events can arrive late, duplicated or malformed. This project shows how I keep a real-time pipeline useful when the input is imperfect and the job has to recover from interruption.",decision:"I separated invalid events into a dead-letter path, used event-time watermarks for bounded state and checkpointed the stream so restart and replay behavior stay explicit.",flow:[{step:"SOURCE",name:"Web events",detail:"JSON events"},{step:"INGEST",name:"Kafka",detail:"Durable stream"},{step:"PROCESS",name:"Spark",detail:"Watermarks + state"},{step:"STORE",name:"Delta Lake",detail:"Idempotent writes"}],signals:["Schema validation","Checkpoint recovery","Late-event handling"],evidence:["Automated Python tests","GitHub Actions CI","Docker Compose environment"],tags:["Kafka","PySpark","Delta Lake","Docker"],url:"https://github.com/Bhanu047/weblog-stream",action:"Explore repository ↗"},
 taxi:{label:"BATCH LAKEHOUSE",title:"NYC Taxi Lakehouse",text:"Monthly public taxi files can change shape, contain invalid records and create duplicate output when a failed load is repeated. This project treats those conditions as design inputs rather than cleanup work.",decision:"I used declared schemas, bronze/silver/gold boundaries and manifest-driven loading. Quality rules stop bad data early, while salted aggregation handles skew without hiding it.",flow:[{step:"SOURCE",name:"NYC TLC",detail:"Monthly trip files"},{step:"BRONZE",name:"Raw + manifest",detail:"Traceable ingestion"},{step:"SILVER",name:"PySpark",detail:"Validated records"},{step:"GOLD",name:"Parquet marts",detail:"Analytics-ready"}],signals:["Idempotent reloads","Schema enforcement","Skew-aware aggregation"],evidence:["Pytest suite","GitHub Actions CI","Documented backfill path"],tags:["PySpark","Parquet","Data Quality","Lakehouse"],url:"https://github.com/Bhanu047/nyc-taxi-lakehouse",action:"Explore repository ↗"},
 retail:{label:"ANALYTICS WAREHOUSE",title:"Retail Warehouse",text:"Retail reporting becomes unreliable when order totals, customer history and incremental loads are defined differently across models. This project creates one tested path from raw records to business-ready marts.",decision:"I separated staging, intermediate and mart layers, kept money in integer cents until presentation, added a three-day incremental lookback and used an SCD Type 2 snapshot for product history.",flow:[{step:"SOURCE",name:"Retail records",detail:"Orders + products"},{step:"STAGE",name:"dbt staging",detail:"Typed inputs"},{step:"MODEL",name:"Star schema",detail:"Facts + dimensions"},{step:"SERVE",name:"Business marts",detail:"Consistent metrics"}],signals:["Revenue reconciliation","Late-arriving records","Historical dimensions"],evidence:["dbt data tests","GitHub Actions CI","Documented lineage"],tags:["SQL","dbt","DuckDB","Dimensional Modeling"],url:"https://github.com/Bhanu047/retail-warehouse",action:"Explore repository ↗"},
 infra:{label:"CLOUD FOUNDATION",title:"Data Platform Terraform",text:"Cloud data platforms become risky when environments drift and pipeline identities receive broad permissions. This project makes the platform repeatable, reviewable and constrained by design.",decision:"I split the platform into reusable data-lake, catalog and identity modules, separated environment inputs and gave the pipeline only the S3 access its data path requires.",flow:[{step:"DEFINE",name:"Terraform",detail:"Reusable modules"},{step:"CHECK",name:"Plan + tests",detail:"Validate changes"},{step:"PROVISION",name:"AWS",detail:"S3 + Glue + IAM"},{step:"OPERATE",name:"Data platform",detail:"Consistent environments"}],signals:["Least-privilege IAM","Environment separation","Plan-time guardrails"],evidence:["Terraform test suite","GitHub Actions CI","No live AWS account required"],tags:["Terraform","AWS","S3","Glue","IAM"],url:"https://github.com/Bhanu047/data-platform-terraform",action:"Explore repository ↗"}
};
const detail=document.getElementById("project-detail");
function render(key){const p=projects[key];const workflow=p.flow.map((stage,index)=>`${index?'<div class="flow-link"><i></i></div>':''}<div class="flow-stage"><small>${String(index+1).padStart(2,'0')} · ${stage.step}</small><b>${stage.name}</b><em>${stage.detail}</em></div>`).join('');detail.innerHTML=`<div class="project-meta"><span class="label">${p.label}</span></div><h3>${p.title}</h3><div class="project-brief"><section><small>PROBLEM</small><p>${p.text}</p></section><section><small>ENGINEERING DECISION</small><p>${p.decision}</p></section></div><div class="workflow-label"><span>ARCHITECTURE FLOW</span><b><i></i> LIVE</b></div><div class="architecture-flow">${workflow}</div><div class="project-evidence"><small>${p.evidenceLabel||"REPOSITORY EVIDENCE"}</small><div>${p.evidence.map(x=>`<span><i>✓</i>${x}</span>`).join("")}</div></div><div class="project-footer"><div class="tags">${p.tags.map(x=>`<span>${x}</span>`).join("")}</div><a href="${p.url}" target="_blank" rel="noreferrer">${p.action||"Explore project ↗"}</a></div>`;}
